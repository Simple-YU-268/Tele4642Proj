    ...
    if room_number in room_auth and room_auth[room_number] == phone_last4:
        ...
        if room_number not in user_data['users']:
            user_data['users'][room_number] = {
                'quota': 0,
                'devices': [],
                'created_at': requests.get('http://worldtimeapi.org/api/timezone/Etc/UTC').json()['unixtime']
            }
        ...
        user_data['sessions'][session_id] = {
            ...
            'login_time': requests.get('http://worldtimeapi.org/api/timezone/Etc/UTC').json()['unixtime']
        }
...
def room_payment():
    ...
    # 简单验证（实际应用中需要更严格的验证）
    if card_number and cvv and expiry_date and len(card_number) >= 12:
    ...
    if room_number in user_data['users']:
        ...
        if user_data['users'][room_number]['quota'] <= 0:
            ...
            for mac in user_data['users'][room_number]['devices']:
                try:
                    requests.post(f'{RYU_CONTROLLER_URL}/removeFromWhitelist', json={'mac': mac})
                except:
                    pass
...
def add_room():
    ...
    if room_number and phone_last4 and len(phone_last4) == 4:
        room_auth = load_room_auth()
